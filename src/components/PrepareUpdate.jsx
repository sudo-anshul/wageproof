...
import {hostedDemo} from '../deployment.js';
...
export default function PrepareUpdate({ caseRecord, revision, stale, historical, readOnly = false, onEditDetails, onSource }) {
  ...
  return <section className="prepare-update" aria-labelledby="prepare-update-title">
    <div className="prepare-heading"><div><span className="eyebrow">Your next useful artifact</span><h2 id="prepare-update-title">Prepare the factual update</h2><p>Report what changed and keep the unanswered questions visible. Downloads save to your computer; nothing is sent or filed.</p></div><span className={`badge ${currentReviewed ? 'good' : 'attention'}`}>{historical ? 'Historical version' : stale ? 'Earlier version · newer sources pending' : currentReviewed ? 'Reviewed for this version' : 'Draft for review'}</span></div>
